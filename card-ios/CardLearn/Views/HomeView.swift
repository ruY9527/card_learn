...
struct HomeView: View {
    ...
    @State private var currentMajor: Int?
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        LazyVStack(spacing: 12) {
                            ForEach(majorList) { major in
                                MajorItem(
                                    major: major,
                                    isSelected: currentMajor == major.majorId
                                ) {
                                    selectMajor(major.majorId)
                                }
                            }
                        }
                        .padding(.horizontal, 16)
                    ...
                    // 无科目提示
                    if subjectList.isEmpty && currentMajor != nil {
                        VStack {
                            Text("该专业暂无科目数据")
                                .font(.system(size: 14))
    ...
    private func fetchData() {
        Task {
            ...
            // 获取推荐卡片
            do {
                recommendCards = try await apiService.getRecommendCards()
            } catch {
                recommendCards = []
            }
    ...
    private func selectMajor(_ majorId: Int) {
        currentMajor = majorId
        subjectList = []
        Task {
            await fetchSubjects(majorId: majorId)
        }
    }
    
    private func navigateToStudy(_ subject: Subject) {
        // 在 SwiftUI 中，我们通过 NavigationLink 来导航
        // 这里使用编程式导航
    }
